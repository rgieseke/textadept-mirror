...
static int l_cf_ta_buffer_new(lua_State *lua),
           ...
           l_cf_pm_remove_browser(lua_State *lua),
...
bool l_init(int argc, char **argv, bool reinit) {
  ...
  lua_newtable(lua);
    ...
    l_cfunc(lua, l_cf_pm_remove_browser, "remove_browser");
...
static int l_cf_pm_remove_browser(lua_State *lua) {
  int index = static_cast<int>(luaL_checkinteger(lua, 1));
  GtkWidget *pm_combo = gtk_widget_get_parent(pm_entry);
  gtk_combo_box_remove_text(GTK_COMBO_BOX(pm_combo), index);
  return 0;
}
